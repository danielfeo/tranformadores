			...
			success: function(data){
				if(data.length > 0){
					...
					for(var i=0; i<data.length; i++){
						...
						texto += '<td>'+data[i].organizacion+'</td><td>'+data[i].email+'</td><td>'+data[i].lenguaje+'</td><td>'+data[i].descripcion+'</td><td>'+data[i].fecha+'</td><td width="3%"><a target="_blank" href="pdf'+data[i].id_experiencia+'-'+data[i].id_categoria+'-'+data[i].id_usuario+'"><i title="Descargar" class="fa fa-download"></i></a></td>';
						texto += '</tr>';
					}
