...
class htmlMimeMail5
{
    ...
    */
    private function encodeHeader($input, $charset = 'ISO-8859-1', $esSubject=false)
    {
			if(!$esSubject)
			{
			...
			{
				//http://stackoverflow.com/questions/1294066/accented-words-in-email-subject-break-spacing-how-do-i-stop-this 
				preg_match_all('/(\w*[\x80-\xFF]+\w*)/', $input, $matches);
				foreach ($matches[1] as $value) {
						$replacement = preg_replace('/([\x80-\xFF])/e', '"=" . strtoupper(dechex(ord("\1")))', $value);
        ...
        switch ($type) {
            case 'mail':
                ...
                if (!empty($this->headers['Subject'])) {
                    $subject = $this->encodeHeader($this->headers['Subject'], $this->build_params['head_charset'], true);
